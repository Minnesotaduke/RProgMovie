...
ggplot(MoviesSeaORD, aes(x=release_season,y=ADJ_budget, fill = release_season)) +
  ...
  labs(title = "Boxplot of release season compared to adjusted production budget", x = "Release Season", y = "production budget adjusted for inflation")+
  theme_minimal()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
ggplot(MoviesSeaORD, aes(x=release_season, y=ADJ_revenue, fill = release_season)) +
  geom_boxplot() +
  scale_y_continuous( trans = "log10",
                      breaks = c(2.5e4, 5e4, 1e5, 2e5, 5e5, 1e6, 2e6, 5e6, 1e7, 2e7, 3.5e7, 5e7, 7.5e7, 1e8, 1.5e8, 2e8, 2.5e8, 5e8, 1e9, 2e9, 3e9, 4e9),
                      labels = function(y) {case_when(
  ...
  labs(title = "Boxplot of release season compared to adjusted worldwide revenue", x = "Release Season", y = "worldwide revenue adjusted for inflation")+
  theme_minimal()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))


#Season V Adjusted Stu Revenue

ggplot(MoviesSeaORD, aes(x=release_season, y=ADJ_studio_revenue_est, fill = release_season)) +
  geom_boxplot(outlier.alpha = 0.2,) +
  scale_y_continuous( trans = "log10",
                      breaks = c(2.5e4, 5e4, 1e5, 2e5, 5e5, 1e6, 2e6, 5e6, 1e7, 2e7, 3.5e7, 5e7, 7.5e7, 1e8, 1.5e8, 2e8, 2.5e8, 5e8, 1e9, 2e9, 3e9 ),
                      labels = function(y) {case_when(
                        y >= 1e9 ~ paste0(y/1e9, "B"),
                        y >= 1e6 ~ paste0(y/1e6, "M"),
                        y >= 1e3 ~ paste0(y/1e3, "K")
  ...
  labs(title = "Boxplot of release season compared to adjusted studio revenue", x = "Release Season", y = "studio revenue adjusted for inflation")+
  theme_minimal()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
HolidayRels <- MoviesReals %>%
  ...
  summarise(AVG_hol_rev = mean(studio_revenue_est))

ggplot(HolidayRels, aes(x = holiday_release, y = AVG_hol_rev,  fill = holiday_release)) +
  geom_bar(stat = "identity") +
  scale_y_continuous(labels = scales::comma) +
  labs(title = "Avg Studio Revenue for a holiday release",
       x = "Is the movie a holiday release?",
       y = "AVG Revenue adjusted for inflation") +
  theme_minimal()
...
ggplot(MoviesReals, aes(x = factor(day_of_month), y = log_revenue)) +
  geom_boxplot() +
  labs(
    title = "Movie Revenue by Day of the Month",
    x = "Day of the Month",
    y = "Log Scale of Worldwide Revenue"
  ) +
  theme_minimal()